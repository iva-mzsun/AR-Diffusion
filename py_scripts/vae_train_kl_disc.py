...
def log_validation(args, model, criterion, valid_dataloader, accelerator, weight_dtype, epoch, global_step):
    ...
    for step, batch in enumerate(valid_dataloader):
        with accelerator.accumulate(model):
            ...
            x_rec, posterior = model(x.to(accelerator.device), sample_posterior=False)
...
def main():
    ...
    model_config.model.params.use_checkpoint = False # Important to avoid MARK_VARIAVLE_TWICE error
    ...
    def initialize_model(model, model_config):
        ...
        if 'encoder.conv_out.weight' in model_state_dict.keys() and model_state_dict['encoder.conv_out.weight'].shape != model.encoder.conv_out.weight.shape:
            if model_state_dict['encoder.conv_out.bias'].shape[0] < model.encoder.conv_out.bias.shape[0]:
                model_state_dict['encoder.conv_out.bias'] = torch.cat([model_state_dict['encoder.conv_out.bias'], 
                                                                torch.zeros_like(model_state_dict['encoder.conv_out.bias'])], dim=0)
                model_state_dict['encoder.conv_out.weight'] = torch.cat([model_state_dict['encoder.conv_out.weight'], 
                                                                torch.zeros_like(model_state_dict['encoder.conv_out.weight'])], dim=0)
                from einops import rearrange                                                            
                model_state_dict['pixel_quantize_conv.weight'] = rearrange(model_state_dict['pixel_quantize.embedding.weight'], 'a (b c d) -> b a c d', c=1,d=1)
            else:
                origin_ch = model_state_dict['encoder.conv_out.weight'].shape[0] // 2
                target_ch = model.encoder.conv_out.weight.shape[0] // 2
                model_state_dict['encoder.conv_out.bias'] = torch.cat([model_state_dict['encoder.conv_out.bias'][:target_ch],
                                                                       model_state_dict['encoder.conv_out.bias'][origin_ch:origin_ch+target_ch]], dim=0)
                model_state_dict['encoder.conv_out.weight'] = torch.cat([model_state_dict['encoder.conv_out.weight'][:target_ch],
                                                                         model_state_dict['encoder.conv_out.weight'][origin_ch:origin_ch+target_ch]], dim=0)
                model_state_dict['decoder.decoder_embed.weight'] = model_state_dict['decoder.decoder_embed.weight'][:, :target_ch]
    ...
    # load criterion
    from src.vae.modules.losses import LPIPSWithDiscriminator
    criterion = LPIPSWithDiscriminator(disc_start=args.discriminator_loss_start_step, 
    ...
    model, criterion, optimizer_G, optimizer_D, train_dataloader, valid_dataloader, lr_scheduler_G, lr_scheduler_D = accelerator.prepare(
        model, criterion, optimizer_G, optimizer_D, train_dataloader, valid_dataloader, lr_scheduler_G, lr_scheduler_D
    )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("3d tokenizer", config=tracker_config,
                                  init_kwargs={"wandb": {"name": os.path.basename(args.exp_dir)}, "group": "vae"})
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                # train discriminator
                if args.discriminator_loss_start_step >= 0 and \
                    global_step >= args.discriminator_loss_start_step and \
                        global_step % 2 == 1:
                    with torch.no_grad():
                        x_rec, posterior = model(x, sample_posterior=True)
                    x = rearrange(x, 'b t c h w -> (b t) c h w')
                    x_rec = rearrange(x_rec, 'b t c h w -> (b t) c h w')
                    d_loss, log_dict = criterion(inputs=x.detach(), 
                    ...
                    "kl_loss": log_dict['train/kl_loss'].detach().item(),
