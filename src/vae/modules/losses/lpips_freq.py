...
class LPIPSWithFreq(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, x_freq, rec_freq, optimizer_idx,
        ...
        rec_loss = self.pixel_loss(inputs.contiguous(), reconstructions.contiguous()).mean()

        freq_loss = 0.0
        for i in range(8):
             freq_loss += torch.abs(x_freq[i] - rec_freq[i])
        freq_loss = freq_loss.mean().to(rec_loss.dtype)
        ...
        if optimizer_idx == 0:
            if self.perceptual_weight > 0:
                ...
                """
                # p_loss = self.perceptual_loss(inputs_2d.contiguous(), reconstructions_2d.contiguous()).mean()
            ...
            # loss = rec_loss + freq_loss + p_loss + g_loss + gan_feat_loss + codebook_loss
            loss = self.pixel_weight * rec_loss + (self.pixel_weight / 8.0) * freq_loss + \
                self.perceptual_weight * p_loss + codebook_loss
