...
from src.modules.dwt.model import DWT_3D, IDWT_3D
...
@torch.no_grad()
def log_validation(args, model, dwt_model, criterion, valid_dataloader, accelerator, weight_dtype, epoch, global_step):
    model.eval()
    criterion.eval()
    pair_imgs = []
    logs_dict = dict({})
    ...
    for step, batch in enumerate(valid_dataloader):
        with accelerator.accumulate(model):
            x = batch["frames"].to(dtype=weight_dtype).to(accelerator.device)
            x_dwt = dwt_model(rearrange(x, 'b t c h w -> b c t h w'))

            x_rec, vq_loss = model(x, x_dwt)
            rec_dwt = dwt_model(rearrange(x_rec, 'b t c h w -> b c t h w'))
            x = rearrange(x, 'b t c h w -> (b t) c h w')
            x_rec = rearrange(x_rec, 'b t c h w -> (b t) c h w')
            ae_loss, log_dict = criterion(codebook_loss=vq_loss, inputs=x, 
                                            reconstructions=x_rec, 
                                            optimizer_idx=0, 
                                            global_step=global_step,
                                            x_freq=x_dwt, rec_freq=rec_dwt,
                                            split="valid")
    ...
    save_x = tensor2img(xs.detach().cpu())
    save_x_rec = tensor2img(xs_rec.detach().cpu())
    try:
    ...
    for k, v in logs_dict.items():
        val_logs["val/" + k] = torch.mean(torch.tensor(v))
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--discriminator_config", type=str, default=None)
    parser.add_argument("--discriminator_loss_type", type=str, default="bce")
    parser.add_argument("--discriminator_loss_weight", type=float, default=0.1)
    parser.add_argument("--discriminator_adaptive_weight", type=bool, default=False)
    parser.add_argument("--discriminator_loss_scheduler", type=str, default="constant")
    parser.add_argument("--discriminator_loss_warmup_steps", type=int, default=None)
    parser.add_argument("--discriminator_loss_start_step", type=int, default=0)
    parser.add_argument("--discriminator_update_frequency", type=int, default=1)
    ...
    parser.add_argument("--learning_rate_D", type=float, default=None)
    parser.add_argument("--lr_scheduler", type=str, default="constant")
    parser.add_argument("--lr_scheduler_D", type=str, default=None)
    ...
    args.learning_rate_D = args.learning_rate_D or args.learning_rate
...
# def get_optimizer(args, model, discriminator2d, discriminator3d):
#     if args.ignore_module_key is None or args.ignore_module_key == []:
#         optim_params_G = model.parameters()
#         optim_params_D = list(discriminator2d.parameters()) + \
#                             list(discriminator3d.parameters())
#     else:
#         optim_params_G, optim_params_D = [], []
#         # filter trainable parameters for model
#         for n, p in model.named_parameters():
#             if_ignore = False
#             for k in args.ignore_module_key:
#                 if k in n:
#                     print(f"Ignore param: {n}")
#                     if_ignore = True
#                     p.require_grad = False
#                     break
#             if not if_ignore:
#                 optim_params_G.append(p)
#         # filter trainable parameters for discriminator
#         for n, p in discriminator2d.named_parameters():
#             if_ignore = False
#             for k in args.ignore_module_key:
#                 if k in n:
#                     print(f"Ignore param: {n}")
...
#                     break
#             if not if_ignore:
#                 optim_params_D.append(p)
#         for n, p in discriminator3d.named_parameters():
#             if_ignore = False
#             for k in args.ignore_module_key:
#                 if k in n:
#                     print(f"Ignore param: {n}")
...
def get_optimizer(args, model, discriminator):
    if args.ignore_module_key is None or args.ignore_module_key == []:
        optim_params_G = model.parameters()
        optim_params_D = discriminator.parameters()
    else:
        optim_params_G, optim_params_D = [], []
        # filter trainable parameters for model
        for n, p in model.named_parameters():
...
def main():
    ...
    # load discriminator
    from src.modules.losses import VQLPIPSWithDiscriminator_wFreq
    criterion = VQLPIPSWithDiscriminator_wFreq(disc_start=args.discriminator_loss_start_step)

    # load optimizer
    optimizer_G, optimizer_D = get_optimizer(args, model, criterion.discriminator)
    # optimizer_G, optimizer_D = get_optimizer(args, model, 
    #                                          criterion.discriminator_2d, 
    #                                          criterion.discriminator_3d)
    ...
    args.lr_scheduler_D = args.lr_scheduler_D \
    ...
    dwt_model = DWT_3D("haar")
    model, dwt_model, criterion, optimizer_G, optimizer_D, train_dataloader, valid_dataloader, lr_scheduler_G, lr_scheduler_D = accelerator.prepare(
        model, dwt_model, criterion, optimizer_G, optimizer_D, train_dataloader, valid_dataloader, lr_scheduler_G, lr_scheduler_D
    )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("3d tokenizer", config=tracker_config,
                                  init_kwargs={"wandb": {"name": os.path.basename(args.exp_dir)}})
    ...
    if args.resume:
        ...
        if resume_ckpt is None:
            ...
            accelerator.load_state(resume_ckpt, model_config=model_config, 
                                   resume_skip_optimizer=args.resume_skip_optimizer,
                                   resume_skip_discriminator=args.resume_skip_discriminator)
        ...
        accelerator.load_state(args.pretrained_ckpt, model_config=model_config, 
                               resume_skip_optimizer=args.resume_skip_optimizer,
                               resume_skip_discriminator=args.resume_skip_discriminator, strict=False)
        initial_global_step = 0
    else:
        initial_global_step = 0
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.validation_steps == 0:
                    val_log = log_validation(args, model, dwt_model, criterion, valid_dataloader, accelerator, weight_dtype, epoch, global_step)
                    log_dict.update(val_log)
            
            progress_bar.set_postfix(**logs)
            log_dict.update({"lr_G": lr_scheduler_G.get_last_lr()[0],
