...
class ResizeDatasetFIDVideoBlur(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        try:
            path = str(self.files[i])
            # print(path)
            if path.endswith('.mp4'): # save by video
                ...
                video_frames = [os.path.join(path, frame) 
...
class DatasetFVDVideoFromFramesResize(torch.utils.data.Dataset):
    ...
    def __init__(self, videos, sample_duration=16, mode='FVD-3DRN50', img_size=112, return_name=False):
        videos = sorted(videos)
        self.video_frames = {}
        for vid in videos:
            vid_name = os.path.basename(vid)
            frames = sorted(os.listdir(vid))
            frames = [os.path.join(vid, frame) for frame in frames]
            self.video_frames[vid_name] = frames
