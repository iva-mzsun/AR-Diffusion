...
class LatentDataset(Dataset):
    def __init__(self, root, fix_prompt, img_mode=False, shuffle=True, max_data_num=None):
        self.img_mode = img_mode
        self.root = root
        self.fix_prompt = fix_prompt
        self.items = [os.path.join(root, item) for item in os.listdir(root) if item.endswith('.npy')]
