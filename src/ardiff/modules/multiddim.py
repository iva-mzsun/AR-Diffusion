...
class DDIMSampler(object):
    ...
    def generate_timestep_matrix(self, num_frames, ar_step, step_template):
        step_matrix, step_index, masks = [], [], []
        ...
        pre_row = torch.zeros(num_frames, dtype=torch.long)

        # 当前矩阵的最后一列还不全为最后一个时间步时，说明还没有生成完毕
        while torch.all(pre_row == num_iterations) == False: 
            # 生成下一列
            new_row = torch.zeros(num_frames, dtype=torch.long)
            ...
            masks.append(new_row != pre_row) # 仅更新步数改变了的视频帧，False: 不用更新， True: 需要更新
            step_index.append(new_row)
            step_matrix.append(step_template[new_row])
            pre_row = new_row
        
        step_mask = torch.stack(masks, dim=0)
        step_index = torch.stack(step_index, dim=0)
        step_matrix = torch.stack(step_matrix, dim=0)
        return step_matrix, step_index, step_mask
    ...
    def sample(self,
        ...
        batch_size, num_frames, n_token_per_frame, n_channel_per_token = shape
        img = x_T if x_T is not None else torch.randn(shape, device=device)

        self.make_schedule(ddim_num_steps=S, ddim_eta=eta, verbose=verbose)
        timestep_template = np.flip(self.ddim_timesteps)
        timestep_matrix, step_index, step_mask = self.generate_timestep_matrix(num_frames, ardiff_step, timestep_template)
        ...
        if verbose:
            ...
            print(f"Step mask: {step_mask}")
        else:
            iterator = list(range(timestep_matrix.shape[0]))

        for i, step in enumerate(iterator):
            update_mask = step_mask[i][None, :, None, None].int().to(device)
            ...
            outs = self.p_sample_ddim(img, conditioning, ts, index=index, update_mask=update_mask,
