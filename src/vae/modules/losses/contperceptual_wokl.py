...
class LPIPSWithDiscriminator_woKL(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=0.5, disc_weight=1.0,
    ...
    def forward(self, inputs, reconstructions, optimizer_idx,
        ...
        # now the GAN part
        if optimizer_idx == 0:
            if self.discriminator_iter_start < 0 or global_step < self.discriminator_iter_start:
