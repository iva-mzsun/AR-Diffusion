...
def build_feature_extractor(mode, root_dir, device=torch.device("cuda"), sample_duration=16):
    ...
    if mode == "clean":
        feat_model = InceptionV3W("tool/eval_fvd", download=True, resize_inside=False).to(device)
        ...
        model_data = torch.load(os.path.join(root_dir, "tool/eval_fvd/resnet-50-kinetics.pth"), map_location='cpu')
        model_state_new = {}
        ...
        feat_model.load_state_dict(torch.load(os.path.join(root_dir, "tool/eval_fvd/i3d_pretrained_400.pt")))
        feat_model = feat_model.to(device).eval()
    else:
