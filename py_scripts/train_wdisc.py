    ...
    for step, batch in enumerate(valid_dataloader):
        with accelerator.accumulate(model):
            ...
            ae_loss, log_dict = criterion(codebook_loss=vq_loss, inputs=x, 
                                            ...
                                            last_layer=model.module.pixel_decoder.conv_out.weight,
                                            # last_layer=model.pixel_decoder.conv_out.weight,
                                            split="valid")
...
def main():
    ...
    # load discriminator
    from src.modules.losses import VQLPIPSWithDiscriminator
    criterion = VQLPIPSWithDiscriminator(disc_start=args.discriminator_loss_start_step)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                # train discriminator
                if global_step >= args.discriminator_loss_start_step and global_step % 2 == 0:
                    with torch.no_grad():
                        x_rec, vq_loss = model(x)
                    x = rearrange(x, 'b t c h w -> (b t) c h w')
                    x_rec = rearrange(x_rec, 'b t c h w -> (b t) c h w')
                    d_loss, log_dict = criterion(codebook_loss=vq_loss, inputs=x, 
                                                reconstructions=x_rec, 
                                                optimizer_idx=1, 
                                                global_step=global_step,
                                                # last_layer=model.pixel_decoder.conv_out.weight,
