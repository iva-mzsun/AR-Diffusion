    ...
    for type in metrics:
        if type.lower() in ['fid-img', 'fvd', 'fid-vid', 'mae', "dtssd"]:
            ...
            if len(gen_inst_names_v) == 0:
                gen_inst_names_v = glob.glob(f"{path_gen}/*.mp4") + glob.glob(f"{path_gen}/*/*.mp4")
                gt_inst_names_v = glob.glob(f"{path_gt}/*.mp4") + glob.glob(f"{path_gt}/*/*.mp4")
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--root_dir', type=str, default="/mnt/bn/lq-mzsun-arnold/codes/video_token/3d_tokenizer")
    parser.add_argument('--root_dir', type=str, default="/mnt/bn/mzsun/codes/3d_tokenizer")
